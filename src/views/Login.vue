	...
	<div class="logon">
		<div class="wrapper fadeInDown">
			<div id="formContent">
				...
				<!-- Login Form -->
				<form>
				<input type="text" id="login" class="fadeIn second" placeholder="Login" v-model="login" @keypress.enter="loginAction">
				<input type="password" id="password" class="fadeIn third" placeholder="Senha" v-model="passwd" @keypress.enter="loginAction">
				<div class="text-right mt-2 mb-3 mr-5 fadeIn fourth">
					<router-link to="/login/esqueci-senha">Esqueci minha senha</router-link>
				</div>
				<input type="button" class="fadeIn fourth" value="Acessar" @keyup.enter.native="loginAction" @click="loginAction">
				</form>
			</div>
		</div>
	</div>
	...
	methods : {
		loginAction() {
			...
			.then((response) => {
				...
				location.href = 'assinatura/consultar'
			}) .catch(() => {
